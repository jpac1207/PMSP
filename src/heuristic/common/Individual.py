...
class Individual:
    def __init__(
        self, number_of_segments, segments_routines, window_size, x=np.array([])
    ):
        self.x = x
        self.cost = 0
        ...
        if self.x.size == 0:
            ...
            for i in range(0, self.number_of_segments):
                for j in range(0, len(self.segments_routines[i])):
                    start_position = randint(
                        0, self.segments_routines[i][j].interval_in_weeks - 1
                    )
                    ...
                    for k in range(0, self.segments_routines[i][j].frequency):
                        ...
                        start_position = (
                            start_position
                            + self.segments_routines[i][j].interval_in_weeks
                        )
                # print(self.x)
